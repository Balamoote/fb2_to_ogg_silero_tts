...
def apply_step_wg(words, gaps, func, disabled, stats, step_name, *args):
    if disabled:
        return words, gaps
    old_text = detokenize_words_gaps(words, gaps)
    new_words, new_gaps = func(words, gaps, *args) if args else func(words, gaps)
    new_text = detokenize_words_gaps(new_words, new_gaps)
    if new_text != old_text:
        stats.add(step_name, 1)
        if stats.logger:
            replacements = find_replacements(old_text, new_text)
            for orig, norm, ctx in replacements:
                stats.logger.add_replacement(step_name, orig, norm, ctx)
    return new_words, new_gaps
...
def normalize_fb2(
    ...
    fast_steps = {1, 2, 3, 21, 24, 27, 29}
    if not fast_steps & disabled_steps:
        print(f"  Быстрый проход: шаги {sorted(fast_steps)} по всему тексту...")
        for item in texts:
            original = item["text"]
            t = original
...
def main():
    ...
    # Если немецкая модель — не чистим умлауты
    yaml_config = {}
    for yaml_path in ["config.yaml", "config.yml"]:
