...
def tokenize_words_gaps(text: str) -> tuple:
    ...
    for part in parts:
        if not part:
            continue
        # Проверяем, это слово или разделитель
...
def detokenize_words_gaps(words: list, gaps: list) -> str:
    """Собирает текст из слов и разделителей."""
    result = []
    for i, (gap, word) in enumerate(zip(gaps, words)):
        result.append(gap)
        result.append(word)
...
class NormalizationLogger:
    ...
    def _clean_old_logs(self):
        if self.log_dir.exists():
            for pattern in ["normalization_*", "tts_en_phrases_*"]:
