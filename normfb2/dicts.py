...
class AcronymDict:
        ...
        try:
            with gzip.open(filepath, "rt", encoding="utf-8") as f:
                for line in f:
                    ...
                    parts = line.split()
                    if len(parts) >= 8:
                        gender, base, nom, gen, dat, acc, ins, prep = (
                            parts[0],
                            parts[1],
                            parts[2],
                            parts[3],
    ...
    def _create_default_regex_dict(self):
        self.regex_dict = {
            r"\bP\.S\.(?![а-яё\w])": "пост скри́птум",
            r"\betc\.(?![а-яё\w])": "эт цетера",
        }
        ...
        try:
            with gzip.open(filepath, "wt", encoding="utf-8") as f:
                ...
                for name in sorted(self.rulers.keys()):
                    ...
                    f.write(
                        f"{forms.get('gender', 'mas')} {forms.get('base', name)} {forms.get('nom', '')} {forms.get('gen', '')} {forms.get('dat', '')} {forms.get('acc', '')} {forms.get('ins', '')} {forms.get('prep', '')}\n"
                    )
        except Exception as e:
    ...
    def get_ruler_form(self, name: str) -> Optional[Dict]:
        ...
        for key, forms in self.rulers.items():
            ...
            key_clean = "".join(
                c
                for c in unicodedata.normalize("NFD", key.lower())
                if not unicodedata.combining(c)
            )
